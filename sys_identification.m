5%% System identification of DC motor with measurements from Arduino
...
validation = 'SequenceData3'; %File must be located in same folder
training = 'SequenceData3';

A_v = readtable(validation);
A_t = readtable(training);

%% We need to rearrange the data on the form iddata(output,input,sampling time)
%training data.

inputVoltage_raw = A_t.Input;  %Input voltage [mV]
outputShaftAngle_raw = A_t.Angle;   %Output shaft angle [deg]
time_raw = A_t.DeltaT;  %Raw time from millis() in Arduino [milliseconds]

%time_raw = time_raw/1000;

inputVoltage = inputVoltage_raw / 1000; % Converting to volt
outputShaftAngle = outputShaftAngle_raw; %Taking absolute value
time = zeros(length(time_raw),1);
Ts_vect = [];

%% Validering data.
t_inputVoltage_raw = A_v.Input;  %Input voltage [mV]
t_outputShaftAngle_raw = A_v.Angle;   %Output shaft angle [deg]
t_time_raw = A_v.DeltaT;  %Raw time from millis() in Arduino [milliseconds]

t_inputVoltage = - t_inputVoltage_raw / 1000; % Converting to volt
t_outputShaftAngle = abs(t_outputShaftAngle_raw); %Taking absolute value
t_time = zeros(length(t_time_raw),1);
t_Ts_vect = [];
